...
class PaperFileDAO extends DAO {
	...
	function &_returnPaperFileFromRow(&$row) {
		...
		$paperFile->setUseCase($row['use_case']);
	...
	function insertPaperFile(&$paperFile) {
		...
		$this->update(
			sprintf('INSERT INTO paper_files
				(' . ($fileId ? 'file_id, ' : '') . 'revision, paper_id, file_name, file_type, file_size, original_file_name, use_case, round, date_uploaded, date_modified, viewable)
				VALUES
				(' . ($fileId ? '?, ' : '') . '?, ?, ?, ?, ?, ?, ?, ?, %s, %s, ?)',
